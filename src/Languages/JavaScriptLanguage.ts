import { Property, TypeData, InstanceData } from '../Struct';
      ...
      if (obj[key].value === undefined) {
        ...
        if (Array.isArray(obj[key].value)) {
          for (let i = 0; i < obj[key].value.length; i++) {
            singleClassContent.push(
              `${obj[key].value[i].x}px ${obj[key].value[i].y}px ${obj[key].value[i].blur}px ${obj[key].value[i].spread}px ${obj[key].value[i].color}`
            );
          }
        } else {
          singleClassContent.push(
            `${obj[key].value.x}px ${obj[key].value.y}px ${obj[key].value.blur}px ${obj[key].value.spread}px ${obj[key].value.color}`
          );
        }
